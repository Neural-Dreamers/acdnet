...
def main():
    ...
    for sr in sr_list:
        convert_sr(os.path.join(dataset_path, 'FSC-22-master', 'audio'),
                   ...
                   sr)
        convert_sr(os.path.join(dataset_path, 'FSC-22-test'),
                   os.path.join(dataset_path, 'test-wav{}'.format(sr // 1000)),
                   sr)
    ...
    # Create npz files for test dataset
    for sr in sr_list:
        src_path = os.path.join(dataset_path, 'test-wav{}'.format(sr // 1000))

        create_test_dataset(src_path, os.path.join(dataset_path, 'test-wav{}.npz'.format(sr // 1000)))
...
def create_test_dataset(src_path, dataset_test_dst_path):
    print('* {} -> {}'.format(src_path, dataset_test_dst_path))
    test_dataset = {'T': {}}
    ...
    for wav_file in sorted(glob.glob(os.path.join(src_path, '*.wav'))):
        ...
        # end = sound.nonzero()[0].max()
        # sound = sound[start: end + 1] # Remove silent sections
        label = int(os.path.splitext(os.path.basename(wav_file))[0].split('_')[0])
        test_sounds.append(sound)
        test_labels.append(label)

    test_dataset['T']['sounds'] = test_sounds
    test_dataset['T']['labels'] = test_labels

    all_labels = {label for fold_data in test_dataset.values() for label in fold_data['labels']}

    # make the labels 1 indexed
    if min(all_labels) == 0:
        for fold_data in test_dataset.values():
            fold_data['labels'] = [label + 1 for label in fold_data['labels']]

    np.savez(dataset_test_dst_path, **test_dataset)
