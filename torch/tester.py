...
class Trainer:
    ...
    def load_test_data(self):
        data = np.load(
            os.path.join(self.opt.data, self.opt.dataset,
                         'test_data_{}khz/fold{}_test4050.npz'.format(self.opt.sr//1000, self.opt.split)),
            allow_pickle=True)
    ...
    def __compute_accuracy(self, y_pred, y_target, lossFunc):
        with torch.no_grad():
            #Reshape to shape theme like each sample contains 10 samples, calculate mean and find the indices that has the highest average value for each sample
    ...
    def TestModel(self, run=1):
        ...
        for f in file_paths:
            ...
            config = state['config']
            weight = state['weight']
            net = models.GetACDNetModel(self.opt.inputLength, 27, self.opt.sr, config).to(self.opt.device)
            ...
            val_acc, val_loss, confusion = self.__validate(net, lossFunc)
