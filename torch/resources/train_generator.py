...
class Generator:
    ...
    def __getitem__(self, batchIndex):
        ...
        return torch.tensor(np.moveaxis(batchX, 3, 1)).float(), torch.tensor(batchY).float()
    ...
    def generate_batch(self, batchIndex):
        ...
        if self.opt.mixupFactor == 2:
            for i in range(self.batch_size):
                ...
                sound_mfcc_spect = u.mfcc_spect(sound, self.opt.sr)

                sounds.append(sound_mfcc_spect)
                labels.append(label)
