...
class ACDNetV2(nn.Module):
    def __init__(self, input_length, n_class, sr, ch_conf=None):
        ...
        nn.init.kaiming_normal_(fcn.weight,
                                nonlinearity='sigmoid')  # kaiming with sigmoid is equivalent to lecun_normal in keras
    ...
    def make_layers(self, in_channels, out_channels, kernel_size, stride=(1, 1), padding=0, bias=False):
        ...
        nn.init.kaiming_normal_(conv.weight,
                                nonlinearity='relu')  # kaiming with relu is equivalent to he_normal in keras
...
def GetACDNetModel(input_len=30225, nclass=26, sr=20000, channel_config=None):
...
class ACDNetQuant(nn.Module):
    def __init__(self, input_length, n_class, sr, ch_conf=None):
        ...
        # tfeb_pool_size = (2,2)
        if self.ch_config is None:
            self.ch_config = [channels, channels * 8, channels * 4, channels * 8, channels * 8, channels * 16,
                              channels * 16, channels * 32, channels * 32, channels * 64, channels * 64, n_class]
        # avg_pool_kernel_size = (1,4) if self.ch_config[1] < 64 else (2,4)
        fcn_no_of_inputs = self.ch_config[-1]
        conv1, bn1 = self.make_layers(1, self.ch_config[0], (1, 9), (1, stride1))
        conv2, bn2 = self.make_layers(self.ch_config[0], self.ch_config[1], (1, 5), (1, stride2))
        ...
        fcn = nn.Linear(fcn_no_of_inputs, n_class)
        nn.init.kaiming_normal_(fcn.weight,
                                nonlinearity='sigmoid')  # kaiming with sigoid is equivalent to lecun_normal in keras
...
def GetACDNetQuantModel(input_len=30225, nclass=26, sr=20000, channel_config=None):
